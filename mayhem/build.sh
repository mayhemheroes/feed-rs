...
# (compiled by libfuzzer-sys via the cc crate) and the from-source std (--build-std,
# so the prebuilt DWARF-5 std isn't linked in) both need explicit -gdwarf-3.
...
# the feed-rs package (unit tests + tests/); --all-features enables the `sanitize`
# feature so tests/sanitize.rs is compiled and run too.
echo "=== building upstream test suite (normal flags) ==="
env -u RUSTFLAGS cargo test --no-run -p feed-rs --all-features
